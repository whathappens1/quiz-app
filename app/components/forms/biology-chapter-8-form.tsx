...
import { questions_chapter_8 as questions } from "@/lib/biology-questions";
...
export default function BiologyChapter5Form() {
  ...
  const [showResults, setShowResults] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter8ShowResults");
  ...
  const [correctAnswers, setCorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter8CorrectAnswers");
  ...
  const [incorrectAnswers, setIncorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter8IncorrectAnswers");
      return saved ? JSON.parse(saved) : 0;
    }
    return 0;
  });
  ...
  const savedAnswers = isClient
    ? Object.values(
        JSON.parse(localStorage.getItem("biologyQuizChapter8Answers") || "{}")
  ...
  const {
    ...
    defaultValues: isClient
      ? {
          ...
          ...JSON.parse(localStorage.getItem("biologyQuizChapter8Answers") || "{}"), // Load saved answers
        }
      : {},
  });
  ...
  useEffect(() => {
    if (isClient) {
      const savedAnswers = JSON.parse(
        localStorage.getItem("biologyQuizChapter8Answers") || "{}"
      );
  ...
  useEffect(() => {
    if (isClient) {
      ...
      if (Object.values(answers).some((value) => value !== "")) {
        localStorage.setItem("biologyQuizChapter8Answers", JSON.stringify(answers));
      }
    }
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem(
        "biologyQuizChapter8ShowResults",
        JSON.stringify(showResults)
      );
      localStorage.setItem(
        "biologyQuizChapter8CorrectAnswers",
        JSON.stringify(correctAnswers)
      );
      localStorage.setItem(
        "biologyQuizChapter8IncorrectAnswers",
  ...
  const resetbiologyQuizChapter8 = () => {
    setShowResults(false);
    setCorrectAnswers(0);
    setIncorrectAnswers(0);
    questions.forEach((q) => setValue(`question-${q.id}`, ""));
    localStorage.removeItem("biologyQuizChapter8Answers");
    localStorage.removeItem("biologyQuizChapter8ShowResults");
    localStorage.removeItem("biologyQuizChapter8CorrectAnswers");
    localStorage.removeItem("biologyQuizChapter8IncorrectAnswers");
  };
  ...
  return (
    ...
    >
      ...
      >
الدروس: التراكيب الخلوية والعضيات,  كيمياء الخلية
      </motion.p>
      <motion.div
        initial={{ opacity: 0, y: -50 }}
        animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4">
              {!showResults ? (
                ...
                <Button onClick={resetbiologyQuizChapter8} className="w-full">
                  إعادة الاختبار
                </Button>
              )}
            </div>
