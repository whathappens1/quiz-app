...
import { questions_chapter_4 as questions } from "@/lib/biology-questions";
...
export default function BiologyChapter4Form() {
  ...
  const [showResults, setShowResults] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter4ShowResults");
  ...
  const [correctAnswers, setCorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter4CorrectAnswers");
  ...
  const [incorrectAnswers, setIncorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter4IncorrectAnswers");
      return saved ? JSON.parse(saved) : 0;
    }
    return 0;
  });
  ...
  const savedAnswers = isClient
    ? Object.values(
        JSON.parse(localStorage.getItem("biologyQuizChapter4Answers") || "{}")
  ...
  const {
    ...
    defaultValues: isClient
      ? {
          ...
          ...JSON.parse(localStorage.getItem("biologyQuizChapter4Answers") || "{}"), // Load saved answers
        }
      : {},
  });
  ...
  useEffect(() => {
    if (isClient) {
      const savedAnswers = JSON.parse(
        localStorage.getItem("biologyQuizChapter4Answers") || "{}"
      );
  ...
  useEffect(() => {
    if (isClient) {
      ...
      if (Object.values(answers).some((value) => value !== "")) {
        localStorage.setItem("biologyQuizChapter4Answers", JSON.stringify(answers));
      }
    }
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem(
        "biologyQuizChapter4ShowResults",
        JSON.stringify(showResults)
      );
      localStorage.setItem(
        "biologyQuizChapter4CorrectAnswers",
        JSON.stringify(correctAnswers)
      );
      localStorage.setItem(
        "biologyQuizChapter4IncorrectAnswers",
  ...
  const resetbiologyQuizChapter4 = () => {
    setShowResults(false);
    setCorrectAnswers(0);
    setIncorrectAnswers(0);
    questions.forEach((q) => setValue(`question-${q.id}`, ""));
    localStorage.removeItem("biologyQuizChapter4Answers");
    localStorage.removeItem("biologyQuizChapter4ShowResults");
    localStorage.removeItem("biologyQuizChapter4CorrectAnswers");
    localStorage.removeItem("biologyQuizChapter4IncorrectAnswers");
  };
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4">
              {!showResults ? (
                ...
                <Button onClick={resetbiologyQuizChapter4} className="w-full">
                  إعادة الاختبار
                </Button>
              )}
            </div>
