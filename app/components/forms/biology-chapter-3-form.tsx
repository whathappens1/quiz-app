...
import { questions_chapter_3 as questions } from "@/lib/biology-questions";
...
export default function BiologyChapter4Form() {
  ...
  const [showResults, setShowResults] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter3ShowResults");
  ...
  const [correctAnswers, setCorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter3CorrectAnswers");
  ...
  const [incorrectAnswers, setIncorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter3IncorrectAnswers");
      return saved ? JSON.parse(saved) : 0;
    }
    return 0;
  });
  ...
  const savedAnswers = isClient
    ? Object.values(
        JSON.parse(localStorage.getItem("biologyQuizChapter3Answers") || "{}")
  ...
  const {
    ...
    defaultValues: isClient
      ? {
          ...
          ...JSON.parse(localStorage.getItem("biologyQuizChapter3Answers") || "{}"), // Load saved answers
        }
      : {},
  });
  ...
  useEffect(() => {
    if (isClient) {
      const savedAnswers = JSON.parse(
        localStorage.getItem("biologyQuizChapter3Answers") || "{}"
      );
  ...
  useEffect(() => {
    if (isClient) {
      ...
      if (Object.values(answers).some((value) => value !== "")) {
        localStorage.setItem("biologyQuizChapter3Answers", JSON.stringify(answers));
      }
    }
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem(
        "biologyQuizChapter3ShowResults",
        JSON.stringify(showResults)
      );
      localStorage.setItem(
        "biologyQuizChapter3CorrectAnswers",
        JSON.stringify(correctAnswers)
      );
      localStorage.setItem(
        "biologyQuizChapter3IncorrectAnswers",
  ...
  const resetbiologyQuizChapter3 = () => {
    setShowResults(false);
    setCorrectAnswers(0);
    setIncorrectAnswers(0);
    questions.forEach((q) => setValue(`question-${q.id}`, ""));
    localStorage.removeItem("biologyQuizChapter3Answers");
    localStorage.removeItem("biologyQuizChapter3ShowResults");
    localStorage.removeItem("biologyQuizChapter3CorrectAnswers");
    localStorage.removeItem("biologyQuizChapter3IncorrectAnswers");
  };
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4">
              {!showResults ? (
                ...
                <Button onClick={resetbiologyQuizChapter3} className="w-full">
                  إعادة الاختبار
                </Button>
              )}
            </div>
