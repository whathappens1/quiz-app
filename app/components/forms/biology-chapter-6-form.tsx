...
import { questions_chapter_6 as questions } from "@/lib/biology-questions";
...
export default function BiologyChapter5Form() {
  ...
  const [showResults, setShowResults] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter6ShowResults");
  ...
  const [correctAnswers, setCorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter6CorrectAnswers");
  ...
  const [incorrectAnswers, setIncorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter6IncorrectAnswers");
      return saved ? JSON.parse(saved) : 0;
    }
    return 0;
  });
  ...
  const savedAnswers = isClient
    ? Object.values(
        JSON.parse(localStorage.getItem("biologyQuizChapter6Answers") || "{}")
  ...
  const {
    ...
    defaultValues: isClient
      ? {
          ...
          ...JSON.parse(
            localStorage.getItem("biologyQuizChapter6Answers") || "{}"
          ), // Load saved answers
        }
      : {},
  });
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem(
        "biologyQuizChapter6ShowResults",
        JSON.stringify(showResults)
      );
      localStorage.setItem(
        "biologyQuizChapter6CorrectAnswers",
        JSON.stringify(correctAnswers)
      );
      localStorage.setItem(
        "biologyQuizChapter6IncorrectAnswers",
  ...
  const resetbiologyQuizChapter6 = () => {
    setShowResults(false);
    setCorrectAnswers(0);
    setIncorrectAnswers(0);
    questions.forEach((q) => setValue(`question-${q.id}`, ""));
    localStorage.removeItem("biologyQuizChapter6Answers");
    localStorage.removeItem("biologyQuizChapter6ShowResults");
    localStorage.removeItem("biologyQuizChapter6CorrectAnswers");
    localStorage.removeItem("biologyQuizChapter6IncorrectAnswers");
  };
  ...
  return (
    ...
    >
      ...
      >
        الدروس: خلايا النبات وانسجته, هرمونات النباتات واستجاباتها{" "}
      </motion.p>
      <motion.div
        initial={{ opacity: 0, y: -50 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <div className="flex items-center gap-2 justify-center">
          ...
          <ModeToggle />
          <a
            href="https://github.com/whathappens1/biologyQuizChapter6-app"
            target="_blank"
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4">
              {!showResults ? (
                ...
                <Button onClick={resetbiologyQuizChapter6} className="w-full">
                  إعادة الاختبار
                </Button>
              )}
            </div>
