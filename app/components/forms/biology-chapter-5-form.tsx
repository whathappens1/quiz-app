...
import { questions_chapter_5 as questions } from "@/lib/biology-questions";
...
export default function BiologyChapter5Form() {
  ...
  const [showResults, setShowResults] = useState<boolean>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter5ShowResults");
  ...
  const [correctAnswers, setCorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter5CorrectAnswers");
  ...
  const [incorrectAnswers, setIncorrectAnswers] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("biologyQuizChapter5IncorrectAnswers");
      return saved ? JSON.parse(saved) : 0;
    }
    return 0;
  });
  ...
  const savedAnswers = isClient
    ? Object.values(
        JSON.parse(localStorage.getItem("biologyQuizChapter5Answers") || "{}")
  ...
  const {
    ...
    defaultValues: isClient
      ? {
          ...
          ...JSON.parse(localStorage.getItem("biologyQuizChapter5Answers") || "{}"), // Load saved answers
        }
      : {},
  });
  ...
  useEffect(() => {
    if (isClient) {
      const savedAnswers = JSON.parse(
        localStorage.getItem("biologyQuizChapter5Answers") || "{}"
      );
  ...
  useEffect(() => {
    if (isClient) {
      ...
      if (Object.values(answers).some((value) => value !== "")) {
        localStorage.setItem("biologyQuizChapter5Answers", JSON.stringify(answers));
      }
    }
  ...
  useEffect(() => {
    if (isClient) {
      localStorage.setItem(
        "biologyQuizChapter5ShowResults",
        JSON.stringify(showResults)
      );
      localStorage.setItem(
        "biologyQuizChapter5CorrectAnswers",
        JSON.stringify(correctAnswers)
      );
      localStorage.setItem(
        "biologyQuizChapter5IncorrectAnswers",
  ...
  const resetbiologyQuizChapter5 = () => {
    setShowResults(false);
    setCorrectAnswers(0);
    setIncorrectAnswers(0);
    questions.forEach((q) => setValue(`question-${q.id}`, ""));
    localStorage.removeItem("biologyQuizChapter5Answers");
    localStorage.removeItem("biologyQuizChapter5ShowResults");
    localStorage.removeItem("biologyQuizChapter5CorrectAnswers");
    localStorage.removeItem("biologyQuizChapter5IncorrectAnswers");
  };
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4">
              {!showResults ? (
                ...
                <Button onClick={resetbiologyQuizChapter5} className="w-full">
                  إعادة الاختبار
                </Button>
              )}
            </div>
